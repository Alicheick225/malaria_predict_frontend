...
from components.styles import ALERT_HIGH, ALERT_LOW, ALERT_MED, inject_css
from utils.api_client import RISK_LEVELS, cached_districts_geojson, cached_districts_table

st.set_page_config(layout="wide", page_title="Carte — MalariaWatch CI", page_icon="🦟")
inject_css()

horizon = st.session_state.get("horizon", 6)

st.title("Carte interactive du risque épidémique")
st.caption(
    f"Niveau de risque prédit pour le mois suivant, par district sanitaire — "
    f"horizon de {horizon} mois (ajustable dans le menu latéral de la page d'accueil)."
)
...
with st.sidebar:
    ...
    for level, color, cls in [
        ("Faible", ALERT_LOW,  "badge-low"),
        ("Modéré", ALERT_MED,  "badge-med"),
        ("Élevé",  ALERT_HIGH, "badge-high"),
    ]:
        st.markdown(f'<span class="{cls}">Risque {level}</span>', unsafe_allow_html=True)
